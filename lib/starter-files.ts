 ...
 *
 * The content lives here, with no filesystem attached, so the desktop app and
 * the browser build create the same wiki. lib/starter.ts writes it to a path;
 * lib/browser-vault.ts writes it through a directory handle.
 *
 * So this is the smallest thing that is genuinely a wiki rather than a demo
...
export type StarterFile = { relPath: string; body: string };

/** Folder names only, for hosts that must create directories explicitly. */
export const STARTER_FOLDERS = FOLDERS.map((f) => f.name);

/** The whole starter wiki as text. Writing it is the caller's problem. */
export function starterFiles(name: string): StarterFile[] {
  const files: StarterFile[] = [];
  const file = (relPath: string, body: string) => {
    files.push({ relPath, body });
  };
